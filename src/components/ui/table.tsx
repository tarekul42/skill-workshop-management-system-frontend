...
function TableRow({ className, ...props }: React.ComponentProps<"tr">) {
  return (
    <tr
      data-slot="table-row"
      className={cn(
        "h-[60px] border-b border-border transition-colors duration-[120ms] ease-linear hover:bg-surface-1 data-[state=selected]:bg-surface-2",
        className,
      )}
      {...props}
    />
...
function TableHead({ className, ...props }: React.ComponentProps<"th">) {
  return (
    <th
      ...
      className={cn(
        "h-11 px-4 text-left align-middle font-body text-xs font-semibold uppercase tracking-[0.04em] text-foreground-muted whitespace-nowrap [&:has([role=checkbox])]:pr-0",
        className,
      )}
      {...props}
    />
