  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      ...
      <div className="rounded-[20px] border border-border bg-surface-1 shadow-raised overflow-hidden">
        ...
        {/* ── Toolbar Row ── */}
        <div className="flex flex-col gap-4 p-4 bg-surface-2 sm:flex-row sm:items-center sm:justify-between border-b border-border">
          <div className="flex items-center gap-3 flex-1 max-w-md">
            {searchKey && (
              <div className="relative flex-1">
                <Search className="absolute top-1/2 left-3.5 size-4 -translate-y-1/2 text-foreground-disabled" />
                <Input
                  ...
                  className="pl-10 h-10 rounded-xl bg-surface-1 border-transparent focus:border-primary/20 transition-all shadow-sm"
                />
              </div>
            )}
            <Button variant="outline" size="sm" className="h-10 rounded-xl bg-surface-1 border-dashed">
          ...
          <div className="flex items-center gap-2">
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" size="icon-sm" className="rounded-lg hover:bg-surface-3">
                  <Settings2 className="size-4" />
                </Button>
              </DropdownMenuTrigger>
              <DropdownMenuContent align="end" className="w-[200px] rounded-xl p-2">
                <p className="px-2 py-1.5 text-xs font-bold text-foreground-muted uppercase tracking-wider">Toggle Columns</p>
                {table
                  .getAllColumns()
                ...
                table.getRowModel().rows.map((row) => (
                  ...
                  >
                    {row.getVisibleCells().map((cell) => (
                      <TableCell key={cell.id} className="px-4 text-sm text-foreground">
                        {flexRender(
          ...
          <div className="flex flex-col items-center justify-between gap-4 p-4 bg-surface-2/50 border-top border-border sm:flex-row">
            <div className="flex items-center gap-6">
              <p className="text-xs font-medium text-foreground-muted">
                Showing <span className="font-bold text-foreground">{table.getState().pagination.pageIndex * pageSize + 1}</span> to{" "}
                <span className="font-bold text-foreground">
                  {Math.min((table.getState().pagination.pageIndex + 1) * pageSize, data.length)}
                </span> of{" "}
                <span className="font-bold text-foreground">{data.length}</span> results
              </p>
              
              <div className="flex items-center gap-2">
                <span className="text-[11px] font-bold text-foreground-disabled uppercase tracking-tighter">Page Size</span>
                <Select
            ...
            <div className="flex items-center gap-1.5">
              ...
              <div className="flex items-center gap-1">
                 ...
                 {table.getPageCount() > 5 && <span className="px-1 text-foreground-disabled">...</span>}
              </div>
