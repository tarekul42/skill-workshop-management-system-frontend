...
import { formatCurrency, formatDate, getInitials, computeDuration, truncate } from "@/lib/formatters";
...
function WorkshopSimilarCard({ workshop }: { workshop: IWorkshop }) {
  return (
    <div className="group block h-full">
      <div className="flex h-full flex-col overflow-hidden rounded-[20px] border border-border bg-surface-1 shadow-raised transition-all duration-300 hover:shadow-float hover:-translate-y-[3px]">
        {/* Image Container */}
        ...
        <div className="flex flex-1 flex-col p-5">
          ...
          <div className="flex items-center justify-between">
            <div className="flex-1">
              {(() => {
                const percentEnrolled = workshop.maxSeats ? workshop.currentEnrollments / workshop.maxSeats : 0;
    ...
    <div className="bg-background min-h-screen">
        ...
        <div className="site-container">
          <nav aria-label="Breadcrumb">
            <ol className="flex items-center gap-2 text-[13px] font-medium text-foreground-muted">
              ...
              <li className="text-foreground-muted/50">/</li>
              <li className="text-foreground line-clamp-1 max-w-[300px]">
        ...
        <div className="grid gap-12 lg:grid-cols-[1fr_380px]">
          ...
          <div className="space-y-12">
            ...
            <div className="mb-2">
              ...
              <div className="relative mb-7 overflow-hidden rounded-[20px]" style={{ height: '440px' }}>
                {workshop.images && workshop.images.length > 0 ? (
                  <>
                    <Image
                      src={workshop.images[0]}
                    ...
                    {workshop.images.length > 1 && (
                      <div className="absolute bottom-4 left-1/2 flex -translate-x-1/2 gap-2 rounded-lg bg-background/90 p-2 backdrop-blur-md">
                        {workshop.images.slice(0, 5).map((img: string, idx: number) => (
                          <div key={idx} className="relative size-16 cursor-pointer overflow-hidden rounded-lg opacity-60 transition-opacity hover:opacity-100">
                            <Image
                              src={img}
              ...
              <div className="mt-6 flex flex-wrap items-center gap-3">
                ...
                {workshop.startDate && workshop.endDate && (
                  <div className="flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3.5 py-2 text-sm">
                    ...
                    <span className="text-foreground">{computeDuration(workshop.startDate, workshop.endDate)}</span>
                  </div>
                )}
                <div className="flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3.5 py-2 text-sm">
                ...
                <div className="flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3.5 py-2 text-sm">
                  <span className="font-semibold text-foreground">{getLevelName(workshop.level)}</span>
                </div>
                <div className="flex items-center gap-1.5 rounded-lg border border-border bg-surface-2 px-3.5 py-2 text-sm">
                  <span className="font-semibold text-foreground">{formatCurrency(workshop.price ?? 0)}</span>
                </div>
              </div>
            </div>
            ...
            {/* About This Workshop */}
               <section>
                 <h2 className="font-display text-2xl font-bold text-foreground mb-4">About This Workshop</h2>
               ...
               {workshop.prerequisites?.length > 0 && (
                 <section>
                   <h2 className="font-display text-2xl font-bold text-foreground mb-4">Prerequisites</h2>
                   <Separator className="mb-4" />
               ...
               {workshop.syllabus?.length > 0 && (
                 <section>
                   <h2 className="font-display text-2xl font-bold text-foreground mb-4">Syllabus</h2>
                   <Separator className="mb-4" />
                   <div className="space-y-4">
                     {workshop.syllabus.map((item, i) => (
                       <div key={i} className="flex items-start gap-3.5 border-t border-border pt-4 first:border-0 first:pt-0">
               ...
               {/* Instructor Card */}
               <section>
                 <h2 className="font-display text-2xl font-bold text-foreground mb-4">Your Instructor</h2>
                 <Separator className="mb-4" />
                 <div className="rounded-2xl border border-border bg-surface-1 p-7">
                   <div className="flex items-start gap-5">
                     <Avatar className="size-20">
                       {typeof workshop.createdBy === 'object' && 'picture' in workshop.createdBy && (workshop.createdBy as { picture?: string }).picture && (
                         <AvatarImage src={(workshop.createdBy as { picture?: string }).picture as string} alt={getCreatorName(workshop.createdBy)} />
                       )}
                     ...
                     <div>
                       ...
                       <p className="text-sm font-medium text-primary mt-1">Expert Instructor</p>
            ...
            {/* Right Column: Sticky Sidebar - THE CONVERSION ENGINE */}
             <aside className="lg:sticky lg:top-[92px] lg:self-start lg:max-h-[calc(100vh-100px)] lg:overflow-y-auto hidden-scrollbar">
               <div className="rounded-[24px] border border-border bg-surface-1 p-7 shadow-2">
                 {/* Price Block */}
                 <div className="mb-6">
                   <p className="text-[13px] font-semibold tracking-wider text-foreground-muted uppercase mb-1">Price</p>
                   <div className="flex items-baseline gap-1.5">
                     <span className="text-5xl font-extrabold tracking-[-0.03em] text-foreground font-display">
                       {workshop.price ? formatCurrency(workshop.price).replace(/BDT|\$/g, '').trim() : 'Free'}
                     </span>
                     {workshop.price && <span className="text-base font-semibold text-foreground-subtle">৳</span>}
                   </div>
                   <p className="text-[13px] text-foreground-muted mt-1">per student</p>
                 </div>
                 ...
                 {/* Seats Block */}
                 <div className="mb-6">
                   <p className="mb-3 text-[13px] font-semibold tracking-[0.04em] uppercase text-foreground-muted">Seats Available</p>
                   {(() => {
                     const percentEnrolled = workshop.maxSeats ? workshop.currentEnrollments / workshop.maxSeats : 0;
                     const bgClass = percentEnrolled <= 0.5 ? 'bg-success' : percentEnrolled <= 0.75 ? 'bg-warning' : 'bg-danger';
                     return (
                       <>
                         ...
                         <p className="mt-3 text-[14px] font-medium">
                           {seatsAvailable > 0 ? (
                             ...
                             <span className="font-bold text-danger">WORKSHOP FULL</span>
                           )}
                           {seatsAvailable > 0 && seatsAvailable <= 5 && (
                             <span className="ml-2 text-danger font-medium animate-pulse">⚠ Almost full!</span>
                           )}
                         </p>
                       </>
                     );
                 ...
                 <div className="mb-6">
                   <EnrollButton
                     ...
                     variant="default"
                     size="lg"
                     className="w-full h-[52px] rounded-xl text-base font-semibold shadow-amber-glow hover:shadow-amber-glow/80 hover:-translate-y-0.5 transition-all"
                   />
                 </div>
                 ...
                 <div className="mb-6 space-y-3">
                   <div className="flex justify-between items-center">
                     <p className="text-[13px] font-semibold tracking-[0.04em] uppercase text-foreground-muted">Starts</p>
                     <p className="font-bold text-foreground text-sm">{workshop.startDate ? formatDate(workshop.startDate) : 'TBA'}</p>
                   </div>
                   <div className="flex justify-between items-center">
                     <p className="text-[13px] font-semibold tracking-[0.04em] uppercase text-foreground-muted">Ends</p>
                     <p className="font-medium text-foreground text-sm">{workshop.endDate ? formatDate(workshop.endDate) : 'TBA'}</p>
                   </div>
                 </div>
                 ...
                 {/* Share Section */}
                 <div>
                   <p className="mb-3 text-xs font-bold tracking-[0.04em] uppercase text-foreground-muted">Share this workshop</p>
                   <div className="flex gap-2">
                     <Button variant="ghost" size="icon-xs" className="rounded-full" aria-label="Copy Link">
                       <Share2 className="size-3.5" />
                     </Button>
                     <Button variant="ghost" size="icon-xs" className="rounded-full" aria-label="Share on Facebook">
                       <span className="text-xs font-bold">f</span>
                     </Button>
                     <Button variant="ghost" size="icon-xs" className="rounded-full" aria-label="Share on LinkedIn">
      ...
      <div className="fixed bottom-0 left-0 right-0 z-40 border-t border-border bg-background/95 p-4 backdrop-blur-md lg:hidden">
        <div className="site-container flex items-center justify-between">
          <div>
            <p className="font-display text-xl font-bold text-foreground">{formatCurrency(workshop.price ?? 0)}</p>
